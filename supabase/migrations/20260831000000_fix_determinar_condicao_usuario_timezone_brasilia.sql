-- Corrige o bug de timezone em determinar_condicao_usuario.
--
-- O timezone da sessão do Postgres no Supabase é UTC. A checagem
--   v_assinatura.data_fim::date < CURRENT_DATE
-- usa a data UTC de "hoje", que já virou para o dia seguinte entre 21:00 e
-- 23:59 no horário de Brasília (America/Sao_Paulo, offset fixo -03:00 desde o
-- fim do horário de verão em 2019). Nessa janela de ~3h, um trial sem cartão
-- que vence HOJE (Brasília) era marcado como 'trial_sem_cartao_expirado'
-- cedo demais.
--
-- Mudança: troca CURRENT_DATE por (now() AT TIME ZONE 'America/Sao_Paulo')::date.
-- Nada mais no corpo da função muda. `data_fim::date` continua truncando em
-- UTC (mesmo comportamento de hoje) — só a referência de "hoje" passa a ser
-- o dia de calendário do Brasil.
--
-- Efeito colateral esperado: NENHUM fora da janela 21:00-23:59 BRT. Durante
-- essa janela, contas cujo data_fim::date == hoje(BRT) deixam de ser
-- consideradas expiradas até a meia-noite local (comportamento correto).
...
BEGIN
  ...
  IF v_assinatura.plano_tipo = 'demonstracao' OR
    ...
    -- FIX timezone: "hoje" no fuso de Brasília, não a data UTC do servidor.
    IF v_assinatura.data_fim IS NOT NULL AND v_assinatura.data_fim::date < (now() AT TIME ZONE 'America/Sao_Paulo')::date THEN
      RETURN 'trial_sem_cartao_expirado';
    END IF;
...
COMMENT ON FUNCTION public.determinar_condicao_usuario IS 'Determina a condição atual do usuário para automação do CRM. Prioridades: reembolso > cancelado > pagamento_falhou > converteu > pagante > trial_com_cartao > trial_sem_cartao > novo. "Hoje" para trial sem cartão usa o fuso America/Sao_Paulo.';
