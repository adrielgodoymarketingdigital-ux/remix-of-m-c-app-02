-- Recriar view produtos_catalogo SEM filtro de quantidade
...
-- Recriar view pecas_catalogo SEM filtro de quantidade
CREATE OR REPLACE VIEW public.pecas_catalogo
WITH (security_invoker=on) AS
SELECT p.id,
...
-- Recriar view dispositivos_catalogo SEM filtro de quantidade (manter filtro vendido=false)
CREATE OR REPLACE VIEW public.dispositivos_catalogo
WITH (security_invoker=on) AS
SELECT d.id,
    d.tipo,
    d.marca,
