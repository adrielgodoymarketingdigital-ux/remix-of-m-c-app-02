...
import { LayoutVendasConfig, Layout80mmVendasConfig } from "@/types/configuracao-loja";
import { Preview80mmVendas } from "./Preview80mmVendas";

interface DialogConfiguracaoLayoutVendasProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

const CONFIG_80MM_PADRAO: Layout80mmVendasConfig = {
  mostrar_logo: true,
...
const LAYOUT_PADRAO: LayoutVendasConfig = {
  formato_papel: "a4",
  tamanho_fonte: "normal",
  config_80mm: CONFIG_80MM_PADRAO,
};

export function DialogConfiguracaoLayoutVendas({
  open,
  onOpenChange,
}: DialogConfiguracaoLayoutVendasProps) {
  const { config, atualizarConfiguracao } = useConfiguracaoLoja();
  const [layout, setLayout] = useState<LayoutVendasConfig>(LAYOUT_PADRAO);
  const [salvando, setSalvando] = useState(false);

  const config80mm: Layout80mmVendasConfig = layout.config_80mm || CONFIG_80MM_PADRAO;
  const isThermal = layout.formato_papel === "80mm" || layout.formato_papel === "58mm" || layout.formato_papel === "personalizado";

  useEffect(() => {
    if (config?.layout_vendas_config) {
      const saved = config.layout_vendas_config as LayoutVendasConfig;
      setLayout({
  ...
  const handleSalvar = async () => {
    ...
    try {
      const sucesso = await atualizarConfiguracao({
        layout_vendas_config: layout as any,
      });
      if (sucesso) {
        toast.success("Configurações de layout de Vendas salvas!");
        onOpenChange(false);
      } else {
  ...
  const handleToggle80mm = (campo: keyof Layout80mmVendasConfig) => {
    setLayout((prev) => ({
      ...prev,
      config_80mm: {
        ...CONFIG_80MM_PADRAO,
  ...
  const secoes80mm: { id: keyof Layout80mmVendasConfig; label: string; desc: string }[] = [
    ...
    { id: "mostrar_itens", label: "Itens da venda", desc: "Produtos/dispositivos vendidos" },
    { id: "mostrar_subtotal", label: "Subtotal", desc: "Valor antes dos descontos" },
    { id: "mostrar_descontos", label: "Descontos", desc: "Desconto manual e cupom" },
    { id: "mostrar_total", label: "Valor total", desc: "Valor final em destaque" },
    { id: "mostrar_forma_pagamento", label: "Forma de pagamento", desc: "PIX, cartão, dinheiro, etc." },
    { id: "mostrar_parcelas", label: "Parcelas", desc: "Detalhes de parcelamento" },
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Configurações de Layout - Vendas
          </DialogTitle>
          <DialogDescription>
            Personalize o layout de impressão do recibo de vendas.
          </DialogDescription>
        </DialogHeader>
