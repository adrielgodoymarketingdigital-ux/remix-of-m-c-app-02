...
export function UploadFotosOS({
  ...
  const handleUpload = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        const fileName = `${user.id}/os-${Date.now()}-${Math.random().toString(36).substr(2, 9)}.${fileExt}`;
  ...
  const removerFoto = useCallback(async (urlParaRemover: string) => {
    try {
      ...
      const pathMatch = urlObj.pathname.match(/\/dispositivos-fotos\/(.+)$/);
