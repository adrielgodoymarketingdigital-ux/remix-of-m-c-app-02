...
import { Layout, Save, Image, Eye, CheckCircle2 } from "lucide-react";
...
export function DialogConfiguracaoLayoutOS({
  ...
  const versaoAtual = layout.versao_layout_a4 || "padrao";
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex flex-col gap-6">
          {/* Seletor de versão A4 — só aparece quando A4 está selecionado */}
          {!is80mm && (
            <>
              <div className="space-y-3">
                <div className="text-sm font-medium">Versão do Layout A4</div>
                <div className="grid grid-cols-2 gap-3">
                  {/* Opção Padrão */}
                  <button
                    type="button"
                    onClick={() => setLayout((prev) => ({ ...prev, versao_layout_a4: "padrao" }))}
                    className={`relative rounded-lg border-2 p-2 text-left transition-all focus:outline-none ${
                      versaoAtual === "padrao"
                  ...
                  <button
                    type="button"
                    onClick={() => setLayout((prev) => ({ ...prev, versao_layout_a4: "tech" }))}
                    className={`relative rounded-lg border-2 p-2 text-left transition-all focus:outline-none ${
                      versaoAtual === "tech"
                        ? "border-primary shadow-sm"
                        : "border-border hover:border-muted-foreground/40"
          ...
          <div className="flex flex-col md:flex-row gap-6">
            ...
            {/* Right side - Preview 80mm only (A4 previews ficam no seletor acima) */}
            {is80mm && (
